...
charts/<project>/
  ...
  values-template.yaml         # jinja2 템플릿 (phase 아님)
  <env>/values-<cluster>.yaml  # build.sh 가 env/*.yaml 로 렌더한 결과
```

`phase` 는 `values-<phase>.yaml` 파일에서 찾는다. `values-template.yaml` 은 렌더 소스이므로
phase 로 취급하지 않는다.
...
`values-template.yaml` 을 `env/*.yaml` 마다 렌더해 `charts/<project>/<env>/` 에 저장한다.
